...
void NavEKF::SelectMagFusion()
{
    ...
    if (dataReady) {
        // ensure that the covariance prediction is up to date before fusing data
        if (!covPredStep) CovariancePrediction();
        // If we haven't performed the first airborne magnetic field update, then we use the simple method of declination to maintain heading
        if(!firstMagYawInit) {
            fuseCompass();
        } else {
...
void NavEKF::CovariancePrediction()
{
    ...
    // we calcualte the upper diagonal and copy to take advantage of symmetry
...
void NavEKF::SetFlightAndFusionModes()
{
    ...
    // Wind state estimation disabled in this prototype
