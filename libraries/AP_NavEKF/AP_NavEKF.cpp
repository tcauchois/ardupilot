...
void NavEKF::CovariancePrediction()
{
    ...
    // generate the upper diagonal and copy to the lower diagonal using symmetry
	SF[0] = dvz - dvz_b;
    ...
    // copy upper diagonal to lower diagonal
    for (uint8_t i=0; i<=20; i++)
    {
        for (uint8_t j=i+1; j<=21; j++)
        {
            nextP[j][i] = nextP[i][j];
        }
    }
